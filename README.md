...
注意：（这是我的猜想哈，也是一个极端情况，一般不大会遇到，实在需要用到此情况可以交流）图片下方的自定义内容若有图片或其他需要加载的组件，而这个组件高度并不固定，需通过加载后才确定高度，可能会存在布局偏差。这里建议自定义内容中若像图片这种组件，请设置好高度，当然，基本上用到也都是需要设置好高度的。若实在必须要用到图片不固定高度，则需要等此自定义内容渲染完成后再进行布局。此条说道的情况很极端，不理解、用不到的完全可以跳过，不必纠结。
